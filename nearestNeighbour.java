...
public class nearestNeighbour {
	...
	public static void main(String[] agrs){
		...
		LinkedList<Character> shortestPath = new LinkedList<Character>();
		...
		for(int start = 0; start < 13; start++){
			...
			do{
				...
				if(nextStep >= 0 && nextStep <= 12){
					...
					path.add(board[p[0]][p[1]].toCharArray()[0]);
					board[p[0]][p[1]] = "*";
			...
			System.out.println("The distance of the path starts at point " + (char)('A' + start) + " is " + totalDistance);
		...
		System.out.println("\nThe path starts at " + (char)('A' + shortestStart) + " : " + Arrays.toString(shortestPath.toArray()));
		System.out.println("The shortest distance of all the distances is " + shortestDistance);
		
		setupTheProblem();
		int index = shortestPath.indexOf('A');
		currentStep = shortestPath.get(index) - 65;
		totalDistance = 0;
		...
		do{
			if(index > 12) index = 0;
			int nextStep = shortestPath.get(index) - 65;
			System.out.println("The next position you need to go is: "+ (char)('A' + nextStep));
			if(nextStep >= 0 && nextStep <= 12){
